...
class NewsletterContent extends \Newsletter {
    ...
    protected function _send_version3 ( \DataContainer $objDc ) {
...
<input type="submit" id="send" class="tl_submit" accesskey="s" value="'.specialchars($GLOBALS['TL_LANG']['tl_newsletter']['send'][0]).'" onclick="return confirm(\''. str_replace("'", "\\'", $GLOBALS['TL_LANG']['tl_newsletter']['sendConfirm']) .'\')">
       ...
       protected function _send_version4 ( \DataContainer $objDc ) {
		...
		$objNewsletter = $this->Database->prepare("SELECT n.* FROM tl_newsletter n LEFT JOIN tl_newsletter_channel c ON n.pid=c.id WHERE n.id=?")
										->limit(1)
										->execute($objDc->id);
